...
use Resque_Event;
use ResqueScheduler_InvalidTimestampException;
...
*/
class Scheduler extends \Resque
{
	const VERSION = "0.1";
	...
	{
		return (int)parent::redis()->zcard('delayed_queue_schedule');
	}
	/**
	...
	public static function getDelayedTimestampSize($timestamp)
	{
		...
		return parent::redis()->llen('delayed:' . $timestamp, $timestamp);
	}
    /**
	...
	public static function nextDelayedTimestamp($at = null)
	{
		...
		$items = parent::redis()->zrangebyscore('delayed_queue_schedule', '-inf', $at, array('limit' => array(0, 1)));
		if (!empty($items)) {
			return $items[0];
		}
	...
	public static function nextItemForTimestamp($timestamp)
	{
		...
		$item = json_decode(parent::redis()->lpop($key), true);
